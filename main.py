...
def read_files_and_fit():
    # Prompt the user to choose a starting point for the parameters
    print("Please choose a starting point for the parameters:")
    print("1. Default (P(gender=M)=0, P(gender=F)=1, P(weight>130|gender=M)=0, P(weight>130|gender=F)=1, P(height>55|gender=M)=0.5, P(height>55|gender=F)=0.5)")
    print("2. Option 1 (P(gender=M)=0.7, P(gender=F)=0.3, P(weight>130|gender=M)=0.8, P(weight>130|gender=F)=0.4, P(height>55|gender=M)=0.7, P(height>55|gender=F)=0.3)")
    print("3. Option 2 (P(gender=M)=0.3, P(gender=F)=0.7, P(weight>130|gender=M)=0.2, P(weight>130|gender=F)=0.8, P(height>55|gender=M)=0.3, P(height>55|gender=F)=0.7)")
    choice = input("Enter your choice (1, 2, or 3): ")

    # Set the starting point for the parameters based on the user's choice
    if choice == '1':
        P_G = np.array([0.0, 1.0])
        P_W_G = np.array([[0.0, 1.0], [1.0, 0.0]])
        P_H_G = np.array([[1.0, 0.0], [0.0, 1.0]])
    elif choice == '2':
        P_G = np.array([0.7, 0.3])
        P_W_G = np.array([[0.8, 0.2], [0.4, 0.6]])
        P_H_G = np.array([[0.7, 0.3], [0.3, 0.7]])
    elif choice == '3':
        P_G = np.array([0.3, 0.7])
        P_W_G = np.array([[0.2, 0.8], [0.8, 0.2]])
        P_H_G = np.array([[0.3, 0.7], [0.7, 0.3]])
    else:
        print("Invalid choice. Using default starting point.")
        P_G = np.array([0.5, 0.5])
        P_W_G = np.array([[0.5, 0.5], [0.5, 0.5]])
        P_H_G = np.array([[0.5, 0.5], [0.5, 0.5]])
